...
export default function AuthenticationModule({ onAuthenticated }) {
  ...
  const submit = async (event) => {
    ...
    try {
      if (isForgot) { const data = await authService.requestPasswordReset(form.email); setMessage(data.message); return; }
  ...
  return <main className="auth-page font-sans"><section className="auth-layout"><aside className="brand-panel"><div className="brand"><span className="brand-mark">AF</span><span>ArticleFlow</span></div><div className="brand-copy"><p className="eyebrow">CREATE · PUBLISH · ENGAGE</p><h1>Turn your ideas into stories that matter.</h1><p>Write beautifully, share knowledge, and invite readers to learn through interactive quizzes.</p></div><div className="feature-list"><span>✦ Publish with confidence</span><span>✦ Build engaging quizzes</span><span>✦ Grow your audience</span></div></aside><section className="form-panel"><div className="form-heading"><p className="eyebrow">{isForgot ? 'ACCOUNT RECOVERY' : isRegister ? 'START YOUR JOURNEY' : 'WELCOME BACK'}</p><h2>{isForgot ? 'Reset your password' : isRegister ? 'Create your account' : 'Sign in to ArticleFlow'}</h2><p>{isForgot ? 'Enter your email and we will prepare a one-time reset link.' : isRegister ? 'Choose how you want to experience ArticleFlow.' : 'Continue creating and discovering meaningful content.'}</p></div><form onSubmit={submit}>{isRegister && <div className="two-columns"><label>Display name<input name="name" placeholder="Jane Doe" value={form.name} onChange={update} required minLength="2" /></label><label>Account type<select name="role" value={form.role} onChange={update}><option value="reader">Reader</option><option value="author">Author</option></select></label></div>}<label>Email address<input name="email" type="email" placeholder="you@example.com" value={form.email} onChange={update} required autoComplete="email" /></label>{!isForgot && <label>Password<input name="password" type="password" placeholder="At least 8 characters" value={form.password} onChange={update} required minLength="8" autoComplete={isRegister ? 'new-password' : 'current-password'} /></label>}{!isRegister && !isForgot && <button type="button" className="forgot-link" onClick={() => { setMode('forgot'); setMessage('') }}>Forgot password?</button>}{message && <p className={message.startsWith('If an account') ? 'success-message' : 'error'} role="alert">{message}</p>}<button className="submit-button" disabled={busy}>{busy ? 'Please wait…' : isForgot ? 'Create reset link' : isRegister ? 'Create account' : 'Sign in'} <span>→</span></button></form><p className="switch">{isForgot ? <>Remembered your password? <button type="button" className="link" onClick={() => { setMode('login'); setMessage('') }}>Sign in</button></> : <>{isRegister ? 'Already have an account?' : 'New to ArticleFlow?'} <button type="button" className="link" onClick={switchMode}>{isRegister ? 'Sign in' : 'Create an account'}</button></>}</p></section></section></main>;
}
